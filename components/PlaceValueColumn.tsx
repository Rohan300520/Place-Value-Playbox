...
export const PlaceValueColumn: React.FC<PlaceValueColumnProps> = ({ 
  ...
  return (
    <div className={`flex flex-col rounded-2xl shadow-xl transition-all duration-300 ${isSpotlighted ? 'relative z-20' : ''}`} style={{ backgroundColor: styles.bg }}>
      <h2 className={`font-display text-xl sm:text-2xl font-black text-center p-2 border-b-4`} style={{ color: styles.text, borderColor: styles.border }}>
        {title}
      </h2>
      <div
        onDrop={handleDrop}
        ...
        className={`flex-grow min-h-[160px] sm:min-h-[200px] lg:min-h-[240px] xl:min-h-[300px] p-1 sm:p-2 md:p-3 transition-all duration-300 rounded-b-2xl border-4 border-dashed bg-black/5 ${borderStyle} ${isRegroupingDestination ? 'animate-pulse' : ''} ${isSpotlighted && !isBeingDraggedOver ? 'animate-guide-pulse' : ''} ${isPulsing ? 'animate-column-pulse' : ''}`}
      >
